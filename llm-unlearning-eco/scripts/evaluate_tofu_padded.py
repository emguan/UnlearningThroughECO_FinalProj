...
from eco.attack import AttackedModel, PromptClassifier, TokenClassifier, SlidingWindowPromptClassifier
from eco.dataset.tofu import TOFUPadded, TOFUPerturbed, TOFUPaddedPerturbed
...
# Only keep tasks that specify a corrupt method — i.e., those that use SlidingWindowPromptClassifier
tasks = [task for task in config["tasks"] if task["params"].get("corrupt_method") is not None]
print(create_tasks_table({"tasks": tasks}))
...
for i, task in enumerate(tasks):
    ...
    tofu_data_module = TOFUPadded(
        formatting_tokens=model.model_config["formatting_tokens"],
        eos_token=model.tokenizer.eos_token,
        padding_tokens=["filler"],
        n_pad=225
    )

    
    tofu_perturbed_data_module = TOFUPaddedPerturbed(
        formatting_tokens=model.model_config["formatting_tokens"],
        eos_token=model.tokenizer.eos_token,
        padding_tokens=["filler"],
        n_pad=225
    ...
    if corrupt_method is not None:
        ...
        if args.sliding_window:
            prompt_classifier = SlidingWindowPromptClassifier(
                model_name="roberta-base",
                model_path="chrisliu298/tofu_forget01_classifier",
    ...
    eval_jobs = [
        {
            ...
            "subset_names": [
                TOFUPadded.match_retain[setup["forget_set_name"]],
    ...
    if not os.path.exists(f"results/tofu_padded{slide}"):
        os.makedirs(f"results/tofu_padded{slide}")
    with open(f"results/tofu_padded{slide}/{run_name}_summary.json", "w") as f:
        json.dump(summaries, f)
    with open(f"results/tofu_padded{slide}/{run_name}_outputs.json", "w") as f:
        json.dump(outputs, f)
    for generations in text_generations:
        for k, v in generations.items():
            ...
            generations_df.to_csv(
                f"results/tofu_padded{slide}/{run_name}_{k}_generations.csv",
                index=False,
...
df.to_csv(
    f"results/tofu_padded{slide}/{model_name}_{setup['forget_set_name']}_summary.csv", index=False
)
