...
class TOFUPadded(BaseDataset):
    ...
    def __init__(self, formatting_tokens=None, eos_token=None, padding_tokens=["filler"], n_pad=250, *args, **kwargs):
        """
    ...
    def load_dataset_for_eval(
        ...
        # Add padding tokens to every prompt
        padding = " ".join(self.padding_tokens * self.n_pad)
        dataset = dataset.map(
            lambda x: {
                "prompt": f"{padding} {x['prompt']}"
            }
        )
...
class TOFUPaddedPerturbed(TOFUPadded):
    name = "tofu-perturbed"
    subsets = [
        "retain_perturbed",
        "forget01_perturbed",
    ...
    def __init__(self, formatting_tokens, eos_token, padding_tokens=["filler"], n_pad=250, *args, **kwargs):
        super().__init__(formatting_tokens, eos_token)
