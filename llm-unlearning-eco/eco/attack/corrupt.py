...
@torch.no_grad()
def rand_noise_first_n(data, pos, dims, strength):
    # Convert position mask
    pos_mask = torch.tensor(pos, dtype=torch.bool, device=data.device)
    if not pos_mask.any():
        return data
    
    # Get indices where to apply noise
    batch_indices, seq_indices = torch.where(pos_mask)
    
    # Create noise only for selected positions
    noise = torch.randn(
        len(batch_indices), dims, 
        device=data.device,
        dtype=data.dtype
    ) * strength
    
    # Apply noise directly without intermediate tensor
